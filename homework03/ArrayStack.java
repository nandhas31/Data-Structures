...
public class ArrayStack<T> {
    ...
    public void push(T data) {
        if (data == null) {
            throw new IllegalArgumentException("Data is null but can not be null");
        }
    ...
    public T pop() {
        if (size == 0) {
            throw new NoSuchElementException("The stack is currently empty and no more elements can be removed");
        }
    ...
    /**
     * Helper method that resizes the backing array to double it's size
     */
    private void resize() {
        T[] temp = backingArray;
        backingArray = (T[]) new Object[temp.length * 2];
